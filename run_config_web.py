        ...
        config_groups["基础配置"].update(
            {
                ...
                "AUTO_MODEL_SWITCH": {
                    "value": getattr(config.llm, 'auto_model_switch', False),
                    "type": "boolean",
                    "description": "自动切换模型",
                    "tooltip": "当您选择的模型回复报错的时候会自动切换模型以保证连贯的用户体验"
                },
                "TEMPERATURE": {
            ...
            if os.path.exists(config_path):
                try:
                    with open(config_path, 'r', encoding='utf-8') as f:
                        ...
                        if 'worldbooks' in config_data:
                            saved_worldbooks = config_data['worldbooks']
                except Exception as e:
                    logger.error(f"读取世界书配置失败: {str(e)}")
            
            # 如果配置文件中有世界书，使用配置文件的顺序
            if saved_worldbooks:
                worldbooks = saved_worldbooks
            else:
                # 否则读取默认的worldview.md
                worldview_file_path = os.path.join(ROOT_DIR, 'src/base/worldview.md')
                if os.path.exists(worldview_file_path):
    ...
    try:
        ...
        for key, value in config_data.items():
            ...
            # 处理其他配置项
                       ...
                       'INTENT_API_KEY', 'INTENT_BASE_URL', 'INTENT_MODEL',
                ...
                try:
                    ...
                    if isinstance(value, str):
                        worldbooks_data = json.loads(value)
                    else:
                        worldbooks_data = value
                    
                    config_data['worldbooks'] = worldbooks_data
                    
                    with open(config_path, 'w', encoding='utf-8') as f:
                        json.dump(config_data, f, ensure_ascii=False, indent=2)
                        
                    # 同时更新worldview.md文件（保持兼容性）
                    if worldbooks_data and len(worldbooks_data) > 0:
                        worldview_file_path = os.path.join(ROOT_DIR, 'src/base/worldview.md')
                        with open(worldview_file_path, 'w', encoding='utf-8') as f:
                            # 将所有启用的世界书内容合并
                            combined_content = []
                            for wb in worldbooks_data:
                                if wb.get('enabled', True):
                                    combined_content.append(wb.get('content', ''))
                            f.write('\n\n'.join(combined_content))
                            
                except Exception as e:
                    logger.error(f"保存世界书配置失败: {str(e)}")
            elif key == 'WORLDVIEW':  # 保持向后兼容
                worldview_file_path = os.path.join(ROOT_DIR, 'src/base/worldview.md')
                try:
                    with open(worldview_file_path, 'w', encoding='utf-8') as f:
                        f.write(value)
        ...
        if key in mapping:
            ...
            if key in ['DEEPSEEK_BASE_URL', 'MODEL', 'DEEPSEEK_API_KEY', 'MAX_TOKEN', 'TEMPERATURE', 'AUTO_MODEL_SWITCH']:
                ...
                if key == 'DEEPSEEK_BASE_URL':
                    ...
                    current['categories']['llm_settings']['settings']['auto_model_switch'] = {'value': value, 'type': 'boolean'}
                return
            ...
            # 特殊处理意图识别相关配置
            elif key in ['INTENT_API_KEY', 'INTENT_BASE_URL', 'INTENT_MODEL']:
...
@app.route('/save_worldbooks', methods=['POST'])
def save_worldbooks():
    """保存世界书配置"""
    try:
        data = request.get_json()
        ...
        config_data['worldbooks'] = worldbooks
        ...
        if worldbooks:
            ...
            with open(worldview_file_path, 'w', encoding='utf-8') as f:
                ...
                for wb in worldbooks:
                    if wb.get('enabled', True) and wb.get('content', '').strip():
                        combined_content.append(wb['content'].strip())
                f.write('\n\n'.join(combined_content))
        ...
        logger.error(f"保存世界书失败: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'保存世界书失败: {str(e)}'
        })
