        ...
        try:
            ...
            with open(self.config_path, 'r', encoding='utf-8') as f:
                ...
                self.llm = LLMSettings(
                    ...
                    stream=bool(llm_data.get('stream', {}).get('value', False))
                )
                ...
                self.media = MediaSettings(
                    image_recognition=ImageRecognitionSettings(
                        ...
                        stream=bool(image_recognition_data.get('stream', {}).get('value', False))
                    ),
                    image_generation=ImageGenerationSettings(
                ...
                self.behavior = BehaviorSettings(
                    auto_message=AutoMessageSettings(
                        enabled=bool(auto_message_data.get('enabled', {}).get('value', False)),
                ...
                self.intent_recognition = IntentRecognitionSettings(
                    ...
                    temperature=float(intent_recognition_data.get('temperature', {}).get('value', 0.0))
                )
