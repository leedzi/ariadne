...
function processFormValue(config, key, value) {
    ...
    // 处理布尔值
    else if (key === 'NETWORK_SEARCH_ENABLED' || key === 'WEBLENS_ENABLED' || key === 'AUTO_MODEL_SWITCH') {
...
function saveConfig(config) {
    ...
    if (!config) {
        ...
        // 特别处理世界书数据
        const worldbooksInput = document.getElementById('worldbooks');
        if (worldbooksInput && worldbooksInput.value) {
            try {
                config['worldbooks'] = JSON.parse(worldbooksInput.value);
            } catch (e) {
                console.error('解析世界书数据失败:', e);
                config['worldbooks'] = [];
            }
        }
    }
